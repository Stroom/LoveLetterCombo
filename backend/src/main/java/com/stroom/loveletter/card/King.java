...
import com.stroom.loveletter.card.action.result.KingActionResult;
...
public class King extends Card {
	...
	public King() {
		super(6, "King", TargetType.OTHER, EffectType.INSTANT, PlayRestriction.NONE);
	}
	...
	public CardActionResult performAction(GameRound gameRound, PlayerMove playerMove) {
		KingActionResult actionResult = null;
		if(playerMove.isValid(gameRound)) {
			//MSG: Create message: CurrentPlayer is playing a King
			...
			if(targetPlayer != null) {
				...
				//MSG: CurrentPlayer got targetCard.
				targetPlayer.setCard(currentCard);
				//MSG: TargetPlayer got currentCard.
				actionResult = KingActionResult.of("Success", currentCard, targetCard);
			}
			else {
				//MSG: The player could not target anyone.
				actionResult = KingActionResult.of("NoTarget", null, null);
			}
